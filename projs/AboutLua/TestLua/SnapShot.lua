...
-- Get the string result without overrided __tostring.，--使用最原始的tostring(cObject),而不触发其元表
local function GetOriginalToStringResult(cObject)
	...
	local strName = ""
	local cToString = rawget(cMt, "__tostring")----检测元表中的tostring方法
	if cToString then
...
local function CreateObjectReferenceInfoContainer()
	...
	cContainer.m_cObjectAddressToName = cObjectAddressToName --table[object] = name
...
local function CollectObjectReferenceInMemory(strName, cObject, cDumpInfoContainer) -- 这个函数比较重要
	if not cObject then
		return
	end
	...
	if "table" == strType then --处理table
		...
		if rawget(cObject, "__cname") then --这里的代码不会执行
		...
		for k, v in pairs(cObject) do
			...
			elseif "userdata" == strKeyType then
				...
				CollectObjectReferenceInMemory(strName .. "." .. k, v, cDumpInfoContainer)----print(strKeyType, k)
			end
		end
...
local function CollectSingleObjectReferenceInMemory(strName, cObject, cDumpInfoContainer)
	...
	elseif "function" == strType then
		...
		-- Check if the specified object.可以认为cExistTag[]里面只保存需要检测的cObject,如果当前是需要寻找的cObject,则记录一下别名
		if cExistTag[cObject] and (not cNameAllAlias[cCombinedName]) then
			cNameAllAlias[cCombinedName] = true
		end

		-- Write this info.如果已经遍历到了,则无需再次遍历, 直接退出
		if cAccessTag[cObject] then
			return
		end
...
local function OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, cDumpInfoResultsBase, cDumpInfoResults)
	...
	for i, v in ipairs(cRes) do
		if (not cDumpInfoResultsBase) or (not cRefInfoBase[v]) then--没有dumpBase或者 DumpBase.m_cObjectReferenceCount中不存在该object(进行给你对比)
...
local function OutputMemorySnapshotSingleObject(strSavePath, strExtraFileName, nMaxRescords, cDumpInfoResults)
	...
	i = 0
	for name,_ in pairs(cObjectAliasName) do
		i = i + 1
		if (nMaxRescords > 0) then
			if (i <= nMaxRescords) then
				cOutputer(name .. "\n")
...
local function DumpMemorySnapshotSingleObject(strSavePath, strExtraFileName, nMaxRescords, strObjectName, cObject)
	...
	-- Collect memory info. 这里也是从registry开始
